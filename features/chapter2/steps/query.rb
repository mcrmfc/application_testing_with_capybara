...
When(/^I search for content using Capybara's query API$/) do
  ...
  page.has_selector? 'ul[id=local_results]'
  page.has_xpath? "//ul[@id='local_results']"
...
Then(/^the correct elements are located$/) do
  ...
  page.should have_selector 'ul[id=local_results]'
  page.should have_xpath "//ul[@id='local_results']"
  page.should have_no_selector '#mock_results'
end
